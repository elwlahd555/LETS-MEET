...
export default {
  ...
  methods: {
      ...
      $executeSave : function $executeSave() {
          //const url = 'letsmeet/image/galleryUpload'; // 서버에 올릴땐 이거 써야한다.
      
          //Object To FormData 변환
          var formData = new FormData();
          if(this.iFile != "") {
              formData.append("iFile", this.iFile); // 이미지 파일
              formData.append("mrNo", this.$store.state.uEmail) // 사진첩 번호
          }
          ...
          // axios.post(url, formData, { // 서버에 올릴땐 이거 써야한다.
              axios.post('http://localhost:8000/letsmeet/image/galleryUpload', formData, { // 이건 로컬용이다.
              header: {
